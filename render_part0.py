        ...
        if getattr(dataset, "use_part_moe", False):
            ...
            if not part_label_path:
                ...
                if scene.loaded_iter is not None:
                    candidate_iters.append(scene.loaded_iter)
                candidate_iters.append(part_iter)
                for candidate_iter in dict.fromkeys(candidate_iters):
                    candidate_path = os.path.join(
                        dataset.model_path,
                        "part_labels",
                        f"iteration_{candidate_iter}",
                        "gaussian_part_label.npy",
                    )
                    if os.path.exists(candidate_path):
                        part_label_path = candidate_path
                        break
                if not part_label_path:
