...
GLOBAL_LOG_FILE="$GLOBAL_LOG_DIR/${RUN_TIME}_ZJU-MoCap_${experiment_name}${GLOBAL_LOG_SUFFIX}.log"
...
if [ "$final_eval_only" = "1" ]; then
    ...
    COMMON_TRAIN_ARGS+=(--test_iterations "$part_moe_start_iter" "$iter")
    COMMON_TRAIN_ARGS+=(--save_iterations "$part_moe_start_iter" "$iter")
fi
