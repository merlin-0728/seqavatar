...
def get_smpl_kwargs(smpl_rot, split_name: str, view):
    if smpl_rot is None:
        return {}
    if split_name not in smpl_rot or view.pose_id not in smpl_rot[split_name]:
        return {}
...
def load_mask(mask_dir: Path, image_name: str, data_dir: Path, epsilon_split: float):
    ...
    depth_3dgs_file = data_dir / "depth" / "novelview" / "npy" / f"{image_name}.npy"
    depth_fs_file = data_dir / "render_depth" / "novelview" / "npy" / f"{image_name}.npy"
...
def split_selected_gaussians(gaussians: GaussianModel, selected: torch.Tensor, split_count: int, split_scale: float):
    ...
    new_scaling = gaussians.scaling_inverse_activation(parent_scale.repeat_interleave(split_count, dim=0) / shrink)
...
def process_sequence(seq: str, args, pipeline, device: torch.device, lpips_model):
    ...
    out_dir = args.output_root / seq / "depth_map" / "gaussian_opt_visual"
    mask_dir = args.output_root / seq / "depth_map" / "mask_high_error"
    fs_depth_dir = data_dir / "render_depth" / "novelview" / "filtered_npy4"# 这些改输入的深度
    ...
    bg = torch.tensor([1, 1, 1] if cfg.white_background else [0, 0, 0], dtype=torch.float32, device=device)
    views = scene.getTestCameras()[args.split]
    ...
    for opt_iter in range(1, args.num_iterations + 1):
        iter_dir = out_dir / f"iter_{opt_iter:03d}"
        iter_dir.mkdir(parents=True, exist_ok=True)
        iter_records = []
        total_render_sec = 0.0
        total_frame_sec = 0.0
        pbar = tqdm(views, desc=f"{seq} iter {opt_iter}/{args.num_iterations}", leave=False)
        ...
        for frame_idx, view in enumerate(pbar):
            ...
            smpl_kwargs = get_smpl_kwargs(smpl_rot, args.split, view)
            ...
            if should_split:
                new_points = split_selected_gaussians(gaussians, selected, args.split_count, args.split_scale)
            ...
            if args.save_visuals and (frame_idx % args.save_every == 0):
                save_visuals(iter_dir, view.image_name, rgb, depth, fs_depth, valid)
        ...
        ply_path = iter_dir / f"gaussians_iter{opt_iter:03d}.ply"
        gaussians.save_ply(str(ply_path))

        if iter_records:
            seq_summary[f"iter_{opt_iter:03d}"] = {
...
def collect_iteration_metrics(summary):
    """
    提取单序列三次迭代指标
    """
    iterations = {}
    ...
    for i in range(1, num_iterations + 1):
        ...
        if key not in summary:
            continue
        iter_info = summary[key]
        iterations[key] = {
            "mean_loss_total": iter_info.get("mean_loss_total"),
            "mean_loss_depth": iter_info.get("mean_loss_depth"),
            "mean_psnr": iter_info.get("mean_psnr"),
            "mean_ssim": iter_info.get("mean_ssim"),
            "mean_lpips": iter_info.get("mean_lpips"),
            "mean_fps": iter_info.get("mean_fps"),
            "throughput_fps_render": iter_info.get("throughput_fps_render"),
            "throughput_fps_frame": iter_info.get("throughput_fps_frame"),
            "total_render_sec": iter_info.get("total_render_sec"),
            "total_frame_sec": iter_info.get("total_frame_sec"),
            "total_new_gaussians": iter_info.get("total_new_gaussians"),
            "final_num_gaussians": iter_info.get("final_num_gaussians"),
            "ply": iter_info.get("ply"),
        }
    return iterations
...
def collect_final_sequence_metrics(summary):
    """
    提取单序列最后一次迭代的汇总指标，供 all summary 使用。
    """
    iterations = summary.get("iterations") or {}
    raw_summary = summary.get("raw_summary", summary)
    num_iterations = summary.get("num_iterations", raw_summary.get("num_iterations", 0))
    final_key = None
    final_iter = {}

    for i in range(num_iterations, 0, -1):
        key = f"iter_{i:03d}"
        if key in iterations or key in raw_summary:
            final_key = key
            if key in raw_summary:
                final_iter.update(raw_summary[key])
            if key in iterations:
                final_iter.update(iterations[key])
            break

    return {
        "sequence": summary.get("sequence", raw_summary.get("sequence")),
        "output_dir": summary.get("output_dir", raw_summary.get("output_dir")),
        "num_iterations": num_iterations,
        "num_views": summary.get("num_views", raw_summary.get("num_views")),
        "fps_mode": summary.get("fps_mode", raw_summary.get("fps_mode")),
        "final_iteration": final_key,
        "mean_loss_total": final_iter.get("mean_loss_total"),
        "mean_loss_depth": final_iter.get("mean_loss_depth"),
        "mean_psnr": final_iter.get("mean_psnr"),
        "mean_ssim": final_iter.get("mean_ssim"),
        "mean_lpips": final_iter.get("mean_lpips"),
        "mean_fps": final_iter.get("mean_fps"),
        "throughput_fps_render": final_iter.get("throughput_fps_render"),
        "throughput_fps_frame": final_iter.get("throughput_fps_frame"),
        "total_render_sec": final_iter.get("total_render_sec"),
        "total_frame_sec": final_iter.get("total_frame_sec"),
        "total_new_gaussians": final_iter.get("total_new_gaussians"),
        "final_num_gaussians": final_iter.get("final_num_gaussians"),
        "ply": final_iter.get("ply"),
    }

# ------------------------- parse_args -------------------------
def parse_args():
    parser = argparse.ArgumentParser(description="Depth-guided SeqAvatar Gaussian optimization for DNA-Rendering novelview")
    ...
    parser.add_argument("--split", type=str, default="novelview")
...
def main():
    ...
    if (not args.run_single_sequence) and len(args.sequences) > 1:
        ...
        for seq in args.sequences:
            seq_summary_path = args.output_root / f"{seq}_optimization_summary.json"
        ...
        merged_path = args.output_root / "depth_guided_optimization_summary_all.json"
    ...
    for summary in summaries:
        ...
        full_summary = {
            ...
            "num_views": summary["num_views"],
            "fps_mode": args.fps_mode,
            "elapsed_sec": elapsed,
            "iterations": collect_iteration_metrics(summary),
        }
        seq_summary_path = args.output_root / f"{seq_name}_optimization_summary.json"
