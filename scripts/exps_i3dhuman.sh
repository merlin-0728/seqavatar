...
#   bash scripts/exps_i3dhuman.sh original
#   bash scripts/exps_i3dhuman.sh part_moe_leg
#   bash scripts/exps_i3dhuman.sh part_budget
#   bash scripts/exps_i3dhuman.sh tri
#   bash scripts/exps_i3dhuman.sh tri_part
#   bash scripts/exps_i3dhuman.sh tri_gate
#   bash scripts/exps_i3dhuman.sh part_moe_leg_unknown_route
#   bash scripts/exps_i3dhuman.sh part_moe_leg_unknown_route_partweight
#   bash scripts/exps_i3dhuman.sh part_moe_leg_unknown_route_unknownmix
#   bash scripts/exps_i3dhuman.sh part_moe_leg_unknown_route_nosig
#
# 常用覆盖方式：
#   GPU_id=3 bash scripts/exps_i3dhuman.sh part_moe_leg
#   GPU_id=3 bash scripts/exps_i3dhuman.sh part_moe_leg_unknown_route
#   GPU_id=3 bash scripts/exps_i3dhuman.sh part_budget
#   GPU_id=3 bash scripts/exps_i3dhuman.sh tri
#   GPU_id=3 bash scripts/exps_i3dhuman.sh tri_part
#   GPU_id=3 bash scripts/exps_i3dhuman.sh tri_gate
#   SEQUENCES_OVERRIDE="ID1_1 ID1_2" GPU_id=3 bash scripts/exps_i3dhuman.sh part_moe_leg
#   PART_MAX_SMPL_DIST=0.05 GPU_id=3 bash scripts/exps_i3dhuman.sh part_moe_leg
#   TRI_PLANE_DIM=32 TRI_PLANE_RES=64 TRI_PLANE_EXTENT=1.0 GPU_id=3 bash scripts/exps_i3dhuman.sh tri
#
...
case "$MODE" in
    ...
    part_moe_leg_unknown_route_partweight)
        experiment_name=part_moe_leg_unknown_route_partweight
        part_moe_enabled=1
        part_score_route_enabled=1
        part_label_schema=part_moe_leg
        num_parts=7
        part_score_route_use_route_gate=1
        part_score_route_use_unknown_mix=0
        part_score_route_signal_mode=full
        ;;
    part_moe_leg_unknown_route_unknownmix)
        experiment_name=part_moe_leg_unknown_route_unknownmix
        part_moe_enabled=1
        part_score_route_enabled=1
        part_label_schema=part_moe_leg
        num_parts=7
    ...
    part_moe_leg_unknown_route_nosig)
        experiment_name=part_moe_leg_unknown_route_nosig
        part_moe_enabled=1
        part_score_route_enabled=1
        part_label_schema=part_moe_leg
        num_parts=7
        part_score_route_use_route_gate=1
        part_score_route_use_unknown_mix=1
        part_score_route_signal_mode=unknown_only
        ;;
    *)
        echo "[ERROR] Unknown mode: $MODE"
        echo "        Supported modes: original, part_moe_leg, part_budget, tri, tri_part, tri_gate, part_moe_leg_unknown_route, part_moe_leg_unknown_route_strong, part_moe_leg_unknown_route_partweight, part_moe_leg_unknown_route_unknownmix, part_moe_leg_unknown_route_nosig"
        exit 1
        ;;
esac
...
DATA_PATH=${DATA_PATH:-$REPO_ROOT/I3D-Human}
...
SMPL_VERTEX_SEG_PATH=${SMPL_VERTEX_SEG_PATH:-$REPO_ROOT/smpl_model/smpl_vert_segmentation.json}
...
part_score_route_use_route_gate=${part_score_route_use_route_gate:-${PART_SCORE_ROUTE_USE_ROUTE_GATE:-1}}
part_score_route_use_unknown_mix=${part_score_route_use_unknown_mix:-${PART_SCORE_ROUTE_USE_UNKNOWN_MIX:-1}}
part_score_route_signal_mode=${part_score_route_signal_mode:-${PART_SCORE_ROUTE_SIGNAL_MODE:-full}}
if [ "$MODE" = "part_moe_leg_unknown_route_strong" ]; then
...
if [ "$tri_enabled" = "1" ]; then
    ...
    AUTO_PART_LOG_DIR="$PART_SCORE_ROUTE_LOG_DIR/.auto_${RUN_TIME}_${experiment_name}"
else
...
echo "[INFO] PART_SCORE_ROUTE_USE_ROUTE_GATE: $part_score_route_use_route_gate"
echo "[INFO] PART_SCORE_ROUTE_USE_UNKNOWN_MIX: $part_score_route_use_unknown_mix"
echo "[INFO] PART_SCORE_ROUTE_SIGNAL_MODE: $part_score_route_signal_mode"
...
if [ "$part_moe_enabled" = "1" ]; then
    ...
    if [ "$part_score_route_enabled" = "1" ]; then
        PART_MOE_ARGS+=(
            ...
            --part_score_route_use_route_gate "$part_score_route_use_route_gate"
            --part_score_route_use_unknown_mix "$part_score_route_use_unknown_mix"
            --part_score_route_signal_mode "$part_score_route_signal_mode"
        )
    fi
fi
