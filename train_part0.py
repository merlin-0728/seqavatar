...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    dynamic_part_labels = part_label_start_iter <= 0
    if getattr(dataset, "use_part_moe", False) and opt.densify_until_iter > part_label_start_iter and not dynamic_part_labels:
        raise ValueError(
    ...
    if getattr(dataset, "use_part_moe", False):
        from ablations.part0_moe_controller import build_part_moe_controller
        part_controller = build_part_moe_controller(dataset)
        init_part_moe_before_loop = False
        if part_label_start_iter <= 0:
            part_controller.before_training(scene, gaussians)
            if not gaussians.part_label_enabled:
                raise RuntimeError("[PartMoE] Part labels must be loaded before initializing experts.")
            init_part_moe_before_loop = True
    gaussians.training_setup(opt)
    if getattr(dataset, "use_part_moe", False) and part_label_start_iter <= 0 and init_part_moe_before_loop:
    ...
    for iteration in range(first_iter, opt.iterations + 1):  
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                if (
                    part_controller is not None
                    and getattr(dataset, "use_part_moe", False)
                    and gaussians.part_label_enabled
                ):
                    part_controller.save_current_labels(iteration, scene, gaussians)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 and len(gaussians.get_xyz) < 120000:
                    ...
                    if (
                        ...
                        part_controller.refresh_labels_after_topology_change(iteration, scene, gaussians)
